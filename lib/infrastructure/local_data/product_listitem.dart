import 'package:shopme/infrastructure/models/product_model.dart';
import 'package:shopme/infrastructure/models/store_model.dart';

List<Product> productList = [
  Product(
    name: "A Bathing Ape Bape x IT 30th Anniversary",
    store: Store(
      "Bape Official",
      "Jl. S. Parman Gg. Tomat No.16 A",
      "assets/images/Bape.png",
      "assets/images/320487.jpg",
      false,
    ),
    price: "1.200.000",
    image:
        "assets/images/a-bathing-ape_a-bathing-ape-bape-x-it-30th-anniversary-shark-full-zip-hoodie-jaket-pria_full05.jpg",
  ),
  Product(
    name: "Apple MacBook Pro 2020 Developer Edition",
    store: Store(
      "Apple Indonesia",
      "Jl. R.Suprapto",
      "assets/images/apple.jpg",
      "assets/images/1568133411_956194_1568138794_noticia_normal.jpg",
      true,
    ),
    price: "31.200.000",
    image:
        "assets/images/05cbcw9cp4o0rqbcnvb2ofz-1fit-limsize-1280x720v-1584707541-1-35bb5677454115b1b29b0a0e7290f5da_600x400.jpg",
  ),
  Product(
    name: "Bape Hodie Limited Edition",
    store: Store(
      "Bape Official",
      "Jl. R.Suprapto",
      "assets/images/apple.jpg",
      "assets/images/1568133411_956194_1568138794_noticia_normal.jpg",
      true,
    ),
    price: "1.200.000",
    image: "assets/images/bapehodie.jpg",
  ),
  Product(
    name: "SteelSeries Arctics 5 2020 Edition",
    store: Store(
      "SteelSeries",
      "Jl. S. Parman Gg. Tomat No.16 A",
      "assets/images/Steelseries-logo.jpeg",
      "assets/images/steelseries_banner.jpg",
      false,
    ),
    price: "3.200.000",
    image: "assets/images/c288b272653632dba3a585858b80161c.jpg",
  ),
];

List<Product> trendList = [
  Product(
    name: "Bape Hodie Limited Edition",
    store: Store(
      "Bape Official",
      "Jl. R.Suprapto",
      "assets/images/apple.jpg",
      "assets/images/1568133411_956194_1568138794_noticia_normal.jpg",
      true,
    ),
    price: "1.200.000",
    image: "assets/images/bapehodie.jpg",
  ),
  Product(
    name: "SteelSeries Arctics 5 2020 Edition",
    store: Store(
      "SteelSeries",
      "Jl. S. Parman Gg. Tomat No.16 A",
      "assets/images/Steelseries-logo.jpeg",
      "assets/images/steelseries_banner.jpg",
      false,
    ),
    price: "3.200.000",
    image: "assets/images/c288b272653632dba3a585858b80161c.jpg",
  ),
  Product(
    name: "Apple MacBook Pro 2020 Developer Edition",
    store: Store(
      "Apple Indonesia",
      "Jl. R.Suprapto",
      "assets/images/apple.jpg",
      "assets/images/1568133411_956194_1568138794_noticia_normal.jpg",
      true,
    ),
    price: "31.200.000",
    image:
        "assets/images/05cbcw9cp4o0rqbcnvb2ofz-1fit-limsize-1280x720v-1584707541-1-35bb5677454115b1b29b0a0e7290f5da_600x400.jpg",
  ),
];
